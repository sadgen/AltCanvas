...
## 2026-09-06 会话（标题栏按钮去重 + 常驻全选 + 服务挂起事件分析，`2ac3eda`）

用户反馈三点：
1. 标题栏「✨ 识别标题」与文库「✨ AI 识别与归类」重叠 → **标题栏按钮已删除**
   （元数据编辑弹窗内的手动提取按钮保留，属编辑场景）。文库多选 + 全选本就
   已实现，本次再补**常驻「全选」切换按钮**：chips 行新增，未全选时显示
   “全选”、全选后变“取消全选”，配合每行勾选框与浮动选择栏。
2. **日志分析**：用户 13:50 (UTC) 操作时全部请求 nginx 504（含最基础的
   /auth/session），但服务自 9/5 起未重启、本地日志该时段完全空白、之后
   自行恢复——定性为**宿主机/NFS 瞬断**（疑似 PVE 夜间备份冻结容器），
   非应用缺陷。加固：服务 .env 增加 `UV_THREADPOOL_SIZE=64`，防止 NFS
   读取挂起占满默认 4 个 libuv 线程导致全服务无响应。
3. 识别进度：125 篇中 **53 篇已完成正文识别**（带附件版本标记），主题 3 个
   （量化多因子与风险模型 / 指数构建与市场基准体系 / 默认主题），剩余 72 篇
   继续点击即可增量完成。

## 2026-09-06 会话二（识别≠归类收口：每篇强制归入最高置信宏观大类，`eb95411`）

用户指出 53 篇已命名文献中 42 篇从未归类，且新分类在界面上不可见。根因与修复：

1. **采纳阈值丢弃**：前端仅采纳置信度 ≥0.7 的建议，模型对宏观大类把握不足时
   给出低分 → 文献"有名字、无分类"。改为**每篇取最高置信度建议强制归入**
   （prompt 已禁止拒收，置信度反映契合度），批内按主题分组采纳。
2. **默认主题占用大类名额**：「我的研究主题」计入 MAX_SYSTEM_TOPICS=4，
   宏观大类实际只有 3 个名额。名额统计排除默认主题。
3. **分类不可见**：列表行新增 🗂 主题徽标（最多显示 2 个）；
   AI 运行后 renderTopicWorkspaceSelector 立即同步主题筛选下拉。
4. **增量补归类**：✨ 的跳过条件收紧为"已按正文识别 **且** 已归类"——
   42 篇已命名未归类的文献会被下一次 ✨ 自动补归类（批内重提取正文，
   归类与命名一次完成）。
...
## 2026-09-09 会话（画板功能优化重构：极简工具栏、多篇增量对比融合与卡片高度自适应，`待提交`）

### 用户反馈与核心诉求
用户反馈画板功能按钮较多且语义不直观，提出三项核心改进：
1. **直接绘制导图**：功能清晰易懂；
2. **多篇对比与增量融合**：选择多篇或画板已有内容再读新文献时，基于已有画板补充完善，自动标出矛盾分歧点、共识相同点或补充点，而不是另起一套孤立不相干的卡片堆；
3. **消除卡片大片空白**：自动生成的卡片下方大面积留白，必须自适应内容大小。

### 核心整改与交付内容

1. **服务端尺寸预估校准与紧凑基底** ([`server/canvas-store.mjs`](file:///home/sadgen/Projects/AltCanvas/server/canvas-store.mjs) & [`server/canvas-api.mjs`](file:///home/sadgen/Projects/AltCanvas/server/canvas-api.mjs))：
   - 修正每行汉字字符估算基数（`charsPerLine = Math.max(20, Math.floor((cardWidth - 28) / 11.5))`，贴合 11px 字体实际渲染宽度）；
   - 移除冗余安全留白，将基准固定高度由 76~88px 缩紧至 52~54px，引用块附加基准高度由 36px 缩紧至 16~20px；
   - 覆盖 overview、sections/concepts/claims、expand-related 与 AI synthesis 全节点生成路径。

2. **前端卡片真实内容高度自适应 (`index.html`)**：
   - 新增 `autoFitAllCanvasNodes`：画板卡片渲染后自动读取实际内容真实渲染高度（`offsetHeight / scrollHeight`），未经过手工调整尺寸的卡片自动紧密贴合内容，彻底根除底部大片黑底空白；
   - 自动重新计算并锚定关系连线端点（`renderCanvasEdges`），使连线平滑对齐紧凑卡片垂直中心；
   - 保持 `autoFitCanvasNodeHeight` 的双击交互与测试断言契约不变。

3. **画板顶栏工具栏极简重构 (`index.html`)**：
   - 顶栏精简分区，主按钮语义重塑：画板为空时清晰显示 **「🗺️ 绘制导图」**，画板已有内容时自适应切换为 **「➕ 对比补充」**，明确告知用户增量对比行为；
   - 移除误放在画板工具栏的 `📥 快速导入`（收归文库管理区）与冷门的 `🔖 证据转批注`（每张卡片脚部保留转批注入口）；
   - 整合并明确 AI 分析与辅助工具。

4. **增量文献深度对比与有机知识融合 (`server/canvas-api.mjs` & `server/canvas-store.mjs`)**：
   - 升级 Stage 2 提示词，深度比对新文献与当前画板已有卡片，明确识别 **矛盾点/分歧** (`contradicts` / `context_differs`)、**相同点/共识** (`supports` / `same_method`) 与 **补充视角** (`extends`)；
   - 视觉标识：产生分歧的卡片呈现玫红色高亮（`#f43f5e`）与 `[分歧]` 徽标；共识支撑卡片呈现翠绿色高亮（`#10b981`）与 `[共识]` 徽标；补充卡片呈现天蓝色高亮（`#3b82f6`）与 `[补充]` 徽标；
   - 有机排版：当新卡片与已有卡片存在直接关联时，卡片自动就近放置于关联已有卡片侧方（拒绝整套文献整体平移至远端 `maxX + 80` 孤岛），并建立指向明确的语义连线。

5. **文库多选一键绘制对比导图 (`index.html`)**：
   - 文库多选提示栏新增 **「🗺️ 绘制对比导图」**（单篇勾选时显示「🗺️ 绘制导图」）；
   - 勾选 2~5 篇文献后一键执行批量提取、导图生成与跨文献知识融合对比，直接在画板生成有机聚合的对比分析图。

6. **连线周长边缘交叉锚定与平滑贝塞尔曲线 (`index.html`)**：
   - 彻底解决连线被卡片实体遮盖截断的问题：由原先“中心到中心”的穿透连线重构为**卡片周长边缘交叉锚定 (`getNodePerimeterAnchor`)**，连线严格从源卡片外侧边框出发，精确接入目标卡片外侧边框；
   - 引入平滑三次贝塞尔曲线 (`path`)、定向箭头标记 (`marker-end`) 与关系语义色彩（分歧红、共识绿、补充蓝、默认紫靛），并将 SVG 浮动至卡片上方（`z-index: 12; pointer-events: none`），使图谱关系一览无余、绝不遮挡卡片文字或交互。

7. **一键「🧹 AI 整理画板」服务与前端交互 (`server/canvas-api.mjs` & `index.html`)**：
   - 新增 `POST /canvas/boards/:id/ai/organize`：当用户逐篇加入多篇文献导致卡片增多杂乱时，AI 全局分析画板全量卡片与关系，提炼核心结论、共识点、分歧点与延伸视角，生成置顶的 **「✨ 全景梳理与多篇对比综述」** 卡片；
   - 空间智能聚类：AI 将杂乱卡片按学术主题重新计算 `(x, y)` 聚类排版（2~4 个清晰主题簇，留出呼吸间距），消除相互遮挡，使混乱的几十张卡片瞬间重组为条理分明的知识导图体系。
