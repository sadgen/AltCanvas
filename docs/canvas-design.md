...
- 全文画板采用分层网格而非自由散放：全文概览位于顶层，章节按阅读顺序排列，概念与
  ...
  卡片标题明确显示“全文概览 / 章节 / 概念 / 论点”。每张卡片必须携带一段逐字 PDF
  原文证据；服务端先在整篇正文中核验并纠正模型页码。模型若轻微改写引用，服务端须
  依据卡片语义从 PDF 实际句子中确定性选择最相关原句后再落库，不能保存模型改写文本。
  若引用与正文完全无法匹配（模型杜撰），服务端从卡片声称的页码范围内确定性选取
  一句真实原文作为锚点，保证每张卡片都可高亮定位；仅当整个正文没有可用文本层时
  才允许卡片无引用。不得让单条证据失败废弃整次生成。
...
- 证据可按需转为正式批注（用户要求的显式操作，不自动触发）：单张卡片点“转批注”，
  或工具栏“证据转批注”批量转换画板内全部未转换的 AI 证据。转换在 PDF 字符层定位
  引用矩形后经 reader 批注管理器创建正式高亮批注（沿用既有保存链路写入 Altero，
  批注 comment 标注“AltCanvas 全文理解证据 · 卡片标题”，卡片色区分），随后把卡片
  来源重绑到新批注 key（provenance `node.source_relinked`），页脚显示“已转批注 ✓”
  且该批注不再参与划线自动中译。无文本层导致无法定位的引用记为失败并提示，不创建
  无坐标的空批注。
