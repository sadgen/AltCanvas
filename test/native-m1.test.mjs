...
try {
  ...
  // 11. Multi-topic document association and cross-topic analysis reuse
  const ws1 = reopenedStore.createWorkspace(adminActorKey, { name: 'Topic 1' });
  const ws2 = reopenedStore.createWorkspace(adminActorKey, { name: 'Topic 2' });
  const b1 = reopenedStore.createBoard(adminActorKey, ws1.id, { name: 'Board 1' });
  const b2 = reopenedStore.createBoard(adminActorKey, ws2.id, { name: 'Board 2' });

  const multiDoc = reopenedStore.createDocument(adminActorKey, { title: 'Multi-Topic Paper' });
  const multiAtt = reopenedStore.createAttachment(adminActorKey, multiDoc.id, {
    blobHash: expectedSha256,
    mimeType: 'application/pdf',
    originalFilename: 'multitopic.pdf'
  });

  const td1 = reopenedStore.addTopicDocument(adminActorKey, ws1.id, {
    libraryType: 'native',
    libraryId: 'local',
    itemKey: multiDoc.id,
    attachmentKey: multiAtt.id,
    status: 'accepted',
    origin: 'native_upload'
  });
  const td2 = reopenedStore.addTopicDocument(adminActorKey, ws2.id, {
    libraryType: 'native',
    libraryId: 'local',
    itemKey: multiDoc.id,
    attachmentKey: multiAtt.id,
  ...
  assert.equal(td1.itemKey, multiDoc.id);
  assert.equal(td2.itemKey, multiDoc.id);
  assert.notEqual(td1.id, td2.id, 'Same native document in multiple topics has distinct topic document IDs');

  // Save analysis for native document
  reopenedStore.saveAiSettings(adminActorKey, {
    baseUrl: 'https://api.openai.com/v1',
    model: 'gpt-4o',
    apiKey: 'sk-test-ai-key'
  });

  let aiCallCount = 0;
  const mockAiHandler = createCanvasHandler(reopenedStore, {
    aiCompletion: async (req) => {
      aiCallCount++;
      const system = String(req.messages?.[0]?.content || '');
      if (system.includes('空间画板')) {
        return JSON.stringify({
          title: 'Native Document Analysis',
          overview: 'This is native document text for AI extraction.',
          evidenceQuote: 'This is native document text for AI extraction.',
          evidencePage: 1,
          sections: [{ title: 'Intro', body: 'Native Intro Body', pageStart: 1, pageEnd: 1, evidenceQuote: 'This is native document text for AI extraction.', evidencePage: 1 }],
          concepts: [{ title: 'Robotics', body: 'Machines', pageStart: 1, pageEnd: 1, evidenceQuote: 'This is native document text for AI extraction.', evidencePage: 1 }],
          claims: [{ title: 'Robots work', body: 'Test evidence', pageStart: 1, pageEnd: 1, evidenceQuote: 'This is native document text for AI extraction.', evidencePage: 1 }],
          relations: []
        });
      }
      return 'Chunk summary notes for native document.';
    }
  });

  // First call on Board 1 generates and caches analysis
  const mapRes1 = await call(mockAiHandler, `/canvas/boards/${b1.id}/ai/document-map`, {
    method: 'POST',
    cookie: newCookie,
    body: {
      document: {
      ...
      pages: [{ pageNumber: 1, text: 'This is native document text for AI extraction.' }]
    }
  });
  assert.equal(mapRes1.statusCode, 201);
  assert.equal(aiCallCount, 2, 'Initial analysis must call chunking + synthesis (2 AI calls)');

  // Second call on Board 2 in different topic reuses cached analysis (0 additional AI calls)
  const mapRes2 = await call(mockAiHandler, `/canvas/boards/${b2.id}/ai/document-map`, {
    method: 'POST',
    cookie: newCookie,
    body: {
      document: {
  ...
  assert.equal(mapRes2.statusCode, 201);
